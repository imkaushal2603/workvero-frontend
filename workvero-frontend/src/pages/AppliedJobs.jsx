...
const STATUS_TABS = [
    ...
    { label: 'Inprogress', match: ['SHORTLISTED', 'INTERVIEW'] },
    { label: 'Selected', match: ['HIRED'] },
    { label: 'Rejected', match: ['REJECTED'] },
    { label: 'Pending', match: ['APPLIED'] },
];

const STATUS_BADGE_MAP = {
    APPLIED: { label: 'Pending', className: 'pending' },
    SHORTLISTED: { label: 'Inprogress', className: 'inprogress' },
    INTERVIEW: { label: 'Inprogress', className: 'inprogress' },
...
function AppliedJobs() {
    ...
    useEffect(() => {
        fetchApplications();
    }, [params.page, params.limit, sortBy]);
    ...
    const filteredApplications = applications.filter(app => {
        ...
        const matchesStatus = !activeTabConfig?.match || activeTabConfig.match.includes(app.status);
            ...
            {loading ? <div>Loading...</div> : error ? <div className="error">{error}</div> : (
                <div className="jobs_table jobs_candidate_view">
                    <div className="jobs_table_header">
                        ...
                        <div className='jobs_table_heading'>Applied Date</div>
                    ...
                    {filteredApplications.length > 0 ? filteredApplications.map(app => {
                        ...
                        return (
                            <div className="jobs_table_body" key={app.id}>
                                ...
                                <div className="jobs_table_row">
                                    <p>{app.createdAt ? new Date(app.createdAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : 'N/A'}</p>
                                </div>
                                <div className="jobs_table_row">
                                    <svg
                                        onClick={() => navigate(`/candidate/browse-jobs/${app.jobId}`)}
                    ...
                    }) : <div className="no-jobs">No applications found.</div>}
                </div>
            )}
