...
function ApplicantsView() {
    ...
    const handleStartConversation = async () => {
        try {
            const targetUserId = applicantData?.user?.id;
            const res = await api.post('/messages/conversations', { targetUserId });
            const conversationId = res.data?.conversation?.id;
            navigate(`/employer/messages?conversation=${conversationId}`);
        } catch (err) {
                    ...
                    <div className='job_btns_info'>
                        <div className='job_btn_sticky'>
                            <div className='job_btns'>
                                ...
                                <button className="message-btn" onClick={handleStartConversation}>
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
