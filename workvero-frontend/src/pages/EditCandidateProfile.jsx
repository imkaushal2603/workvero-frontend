...
function EditCandidateProfile() {
    ...
    const handleBasicInfoSubmit = async (e) => {
        ...
        try {
            ...
            if (fileObjects.logo) {
                ...
                const avatarRes = await api.post('/candidate/me/avatar', avatarData, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                });
                if (avatarRes.data.candidate?.photoUrl) {
                    setFormData(prev => ({ ...prev, logo: avatarRes.data.candidate.photoUrl }));
                    setFileObjects(prev => ({ ...prev, logo: null }));
                }
    ...
    const handleAddressSubmit = async (e) => {
        ...
        setIsSubmitting(true);
        try {
            await api.patch('/candidate/me/address', {
